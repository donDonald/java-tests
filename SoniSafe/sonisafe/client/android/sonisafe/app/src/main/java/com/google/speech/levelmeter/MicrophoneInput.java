...
public class MicrophoneInput implements Runnable{
    ...
    public void setBps(int bps) {
        switch (bps) {
            case 8: mAudioFormat = AudioFormat.ENCODING_PCM_8BIT; break;
            case 16: mAudioFormat = AudioFormat.ENCODING_PCM_16BIT; break;
    ...
    public void run() {
        ...
        int buffer1000msSize = bufferSize(
        ...
        try {
            ...
            while (mRunning) {
                ...
                mListener.processAudioFrame(buffer20ms, numSamples);
            }
            recorder.stop();
        } catch (Exception e) {
        } finally {
    ...
    private int bufferSize(
        ...
        if (buffSize < sampleRateInHz) {
            // On emulator AudioRecord.getMinBufferSize might return error.
